...
// Function to save all content locally
function saveAllContent() {
    const content = {
        original: editors[0].getContents(),
        adaCloseThird: editors[1].getContents(),
        melissaFirstPerson: editors[2].getContents(),
        zoeComments: editors[3].getContents(),
        catarinaComments: editors[4].getContents()
    };
    localStorage.setItem('savedContent', JSON.stringify(content));
    console.log('All content saved locally');
}
...
function loadSavedContent() {
    ...
    if (savedContent) {
        ...
        editors[0].setContents(content.original);
        editors[1].setContents(content.adaCloseThird);
        editors[2].setContents(content.melissaFirstPerson);
        editors[3].setContents(content.zoeComments);
        editors[4].setContents(content.catarinaComments);
        console.log('Loaded saved content');
    }
}

// Set up periodic polling and local saving
function startPolling() {
    ...
    setInterval(saveAllContent, 60000); // Auto-save content every 60 seconds
}
...
window.onload = function() {
    ...
    saveButton.textContent = 'Save All Content Locally';
    saveButton.onclick = saveAllContent;
    saveButton.style.marginTop = '10px';
    container.appendChild(saveButton);

    const loadButton = document.createElement('button');
    loadButton.textContent = 'Load Saved Content';
    loadButton.onclick = loadSavedContent;
